...
1. Flash **Raspberry Pi OS (64-bit)** with Raspberry Pi Imager. In the
   imager's settings, set a hostname (e.g. `marquee`), enable SSH, and
   add your Wi-Fi credentials.
2. Boot the Pi, SSH in, and run:

```bash
curl -fsSL https://raw.githubusercontent.com/beastech/marqueemark/main/install.sh | bash
```

The script installs everything, sets up the service, and prints your
next steps (add art via the browser, then calibrate). Reboot when it
finishes. That's the whole install; the sections below describe the
same steps manually, for reference or customized setups.
...
- Detection requires a NeoSD Pro. With a real MVS cartridge in the slot
  the marquee shows the generic art (`--idle generic`) - manual selection
